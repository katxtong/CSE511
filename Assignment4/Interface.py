    ...
    with openconnection.cursor() as cursor:
        ...
        for each_table in tables:
            if not (each_table[0] == 'rangeratingsmetadata' or each_table[0] == 'roundrobinratingsmetadata'):
                sqlSelectCommand = "SELECT * FROM {} WHERE Rating >= {} AND Rating <= {}".format(
                    each_table[0], ratingMinValue, ratingMaxValue)
                cursor.execute(sqlSelectCommand)
                values = cursor.fetchall()

                for each_value in values:
                    newRow = "{},{},{},{}".format(
                        each_table[0], each_value[0], each_value[1], each_value[2])
    ...
    with openconnection.cursor() as cursor:
        ...
        for eachTable in tables:
            if not (eachTable[0] == 'rangeratingsmetadata' or eachTable[0] == 'roundrobinratingsmetadata'):
                sqlSelectCommand = "SELECT * FROM {} WHERE Rating = {}".format(
                    eachTable[0], ratingValue)
                cursor.execute(sqlSelectCommand)
                values = cursor.fetchall()

                for eachValue in values:
                    newRow = "{},{},{},{}".format(
                        eachTable[0], eachValue[0], eachValue[1], eachValue[2])
                    new_list.append(newRow)
