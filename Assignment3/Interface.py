...
def loadRatings(ratingstablename, ratingsfilepath, openconnection):
    ...
    cursor.execute("CREATE TABLE " + ratingstablename +
                   "(userid int not null, movieid int, rating real, timestamp int);")
    with open(ratingsfilepath) as f:
        for nlines in iter(lambda: tuple(islice(f, 5000)), ()):
            batch = StringIO()
            batch.write(''.join(l.replace('::', ',') for l in nlines))
            batch.seek(0)
            cursor.copy_from(batch, ratingstablename, sep=',', columns=(
...
def rangePartition(ratingstablename, numberofpartitions, openconnection):
    ...
    for i in xrange(numberofpartitions):
        if i == 0:
            cursor.execute(createpart_init.format(
                i, i*stepsize, (i+1)*stepsize))
        else:
            cursor.execute(createpart.format(
                i, i * stepsize, (i + 1) * stepsize))
    cursor.close()
...
def roundRobinPartition(ratingstablename, numberofpartitions, openconnection):
    ...
    cursor = openconnection.cursor()
    for i in xrange(numberofpartitions):
        cursor.execute(createrr.format(
            i, ratingstablename, numberofpartitions, i))
    cursor.close()
...
def roundrobininsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    cursor.execute('''INSERT INTO {0} VALUES ({1},{2},{3})'''.format(
    ...
    cursor.execute(
        '''SELECT * FROM information_schema.tables WHERE table_name LIKE 'rrobin_part%' ''')
    numparts = len(cursor.fetchall())

    tbid = (numrecords-1) % numparts

    cursor.execute('''INSERT INTO rrobin_part{0} VALUES ({1},{2},{3})'''.format(
        tbid, userid, itemid, rating))
    cursor.close()
...
def rangeinsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    cursor.execute(
        '''SELECT * FROM information_schema.tables WHERE table_name LIKE 'range_part%' ''')
    numparts = len(cursor.fetchall())
